...
if (pokemon != null)
{
    Console.WriteLine($"\nCongratulations! {pokemon.Nome.ToUpper()} is now yours! Here are some details about your new partner:");
    Console.WriteLine();

    Console.WriteLine($"Name: {pokemon.Nome}");
    Console.WriteLine();
    Console.WriteLine($"Types: {string.Join(", ", pokemon.Tipos.Select(t => t.Type.Name))}");
    Console.WriteLine();
    Console.WriteLine($"Weight: {pokemon.Peso / 10.0} kg");
    Console.WriteLine();
    Console.WriteLine($"Height: {pokemon.Altura/10.0} m");
